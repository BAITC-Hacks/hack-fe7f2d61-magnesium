...
import { apiConfigured } from "@/lib/brief-api";
import { initialWorkspace } from "@/lib/fixtures";
...
const STORAGE_KEY = "skillarena.workspace.v1";
...
export default function Arena() {
    ...
    try {
      ...
      if (stored) {
        ...
        if (isWorkspace(parsed)) setWorkspace(parsed);
          ...
          setStorageError(
            "Сохранённые данные несовместимы. Загружены примеры; изменения будут сохранены заново.",
          );
      }
    } catch {
      setStorageError(
        "Не удалось прочитать сохранённые данные. Демо работает с примерами в текущем окне.",
      );
    }
    setReady(true);
  }, []);
  useEffect(() => {
    if (ready) {
      try {
        ...
        setStorageError(
          "Хранилище браузера недоступно. Изменения сохраняются только до перезагрузки этой страницы.",
        );
      }
    }
  }, [workspace, ready]);
  ...
  const task = workspace.tasks.find((task) => task.id === activeId);
  ...
  function saveTask(task: BusinessTask, publish: boolean) {
    // Published cards can only change through explicit confirmation and publication.
    if (!publish && task.published) return;
    const saved = publish ? task : { ...task, published: false };
    setWorkspace((prev) => ({
      ...prev,
      tasks: [saved, ...prev.tasks.filter((item) => item.id !== task.id)],
    }));
    setActiveId(task.id);
    setView(publish ? "detail" : "business");
    setToast(
      publish
        ? `Задача опубликована · ${scoreTask(task).total}/100 · доступна всем командам`
        : "Черновик сохранён. Он ещё не опубликован в каталоге.",
    );
  }
      ...
      <div className="main-shell">
        <header className="topbar">
          ...
          <div className="topbar-right">
            ...
            <span className="avatar small">Mg</span>
          </div>
        </header>
          ...
          {!ready ? (
            ...
            <BusinessWorkspace
              ...
              onDecision={(id, status) => {
                setWorkspace((prev) => ({
                  ...
                  proposals: prev.proposals.map((proposal) =>
                    proposal.id === id && !proposal.milestoneConfirmed
                      ? { ...proposal, status }
                      : proposal,
                  ),
                }));
                setToast(
                  status === "selected"
                    ? "Команда выбрана вами. Баллы появятся после подтверждения выполненного этапа."
              ...
              onMilestone={(id) => {
                setWorkspace((prev) => ({
                  ...prev,
                  proposals: confirmMilestone(prev.proposals, id),
                }));
...
function Catalog({
  ...
  const xp = teamXp(proposals, "team-you");
  return (
    <div className="enter">
      <section className="catalog-hero">
        <div className="hero-content">
          ...
          <div className="hero-proof">
            ...
            <span>
              <strong>5 команд</strong> уже в демо-арене
      ...
      <div className="catalog-summary">
        ...
        <div>
          ...
          <strong className="mono">05</strong>
      ...
      <div className="catalog-layout">
        ...
        <aside className="catalog-aside">
          <section className="panel team-progress">
            ...
            <span className="team-avatar">Mg</span>
            <h3>Magnesium</h3>
            <p>
              От первой идеи
              <br />
