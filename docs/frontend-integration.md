# Объединённое приложение в main

`pnpm dev` из корня запускает оба сервиса. Next.js проксирует `/api/*` на Fastify; `NEXT_PUBLIC_API_URL` не нужен для локального запуска. `lib/workspace-api.ts` преобразует серверные карточки и предложения в типы интерфейса; `components/arena.tsx` получает каталог, черновики, команды и XP из API. Публикации и черновики хранятся раздельно.

Дополнения при интеграции:

- `POST /api/brief/analyze`: `{brief, fields}` с полями frontend `context`, `need`, `data`, `outcome`, `criteria`, `constraints`, `users`, `contact`, `interaction`. Ответ `{questions, source, ai}`; `source` — `api` или `demo`. Анализ не создаёт черновик и не публикует задачу. Секрет модели хранится на backend.
- `POST /api/tasks` принимает необязательное `company` (до 120 символов); `PATCH /api/tasks/:id` — также `company` и `description` (до 12000). Название компании сохраняется в опубликованном снимке.
